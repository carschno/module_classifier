...
def test_explain():
    expected = {
        3: [
            ("test", pytest.approx(-0.16820062007910383, abs=0.2)),
            ("text", pytest.approx(0.37238473285178575, abs=0.2)),
        ]
    }
    text = "test text"

    explainer = Explainer(Classifier(TEST_MODEL))
    explanation = explainer.explain(text, num_features=10, num_samples=10)
